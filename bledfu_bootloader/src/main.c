...
void main(void)
{
	...
	gpio_pin_configure(gpiob, PIN, DT_ALIAS_SW0_GPIOS_FLAGS | GPIO_INPUT);
	val = gpio_pin_get(gpiob, DT_ALIAS_SW0_GPIOS_PIN);

	LOG_INF("Welcome to ZB-8 bootloader with bledfu [%s]",
		val ? "BP" : "No BP");
